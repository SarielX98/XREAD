<template>
	<view>
		...
		style="height: 100rpx;">
			...
			:class="tabIndex === index?'text-pink font-lg font-weight-bold':''"
		...
		methods: {
			...
			getData(){
				...
				this.$H.get('/topicclass/listtopicclass').then(res=>{
					// 渲染分类
					this.tabBars = res.topiclassList
					var arr = []
			...
			getList(){
				...
				this.$H.get('/topic/topicclass/'+id+'/page/'+page).then(res=>{
					let list = res.listTopic.list.map(v=>{
						return {
							id:v.id,
							cover:v.titlepic,
							title:v.title,
