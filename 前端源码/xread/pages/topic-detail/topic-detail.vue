<template>
	<view>
		...
		:key="index">
			<text class="iconfont iconcreditlevelfilling text-main"></text>
		...
		<view class="flex align-center py-2">
			...
			v-for="(item,index) in tarBars" :key="index"
			:class="index === tabIndex ? 'font-lg font-weight-bold text-pink' : 'font-md'"
	...
	export default {
		...
		data() {
			return {
				...
				hotList:[{
					title:"【新人必读】各位书友小伙伴可以在这个专题中写下自己的原创小说"
				},{
					title:"【新人必读】各位书友小伙伴可以在这个专题中写下自己的原创小说"
				}],
		...
		methods: {
			...
			getData(){
				...
				this.$H.get('/post/getpostbytopic?topic_id='+this.info.id+'&page='+page,{},{
					token:true,
					noCheck:true
				}).then(res=>{
					let list = res.postList.list.map(v=>{
